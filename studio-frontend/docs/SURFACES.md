...
|---|---|---|---|
| `lens.evidence` | `src/features/lens/Lens.tsx`, inside the `lens-context` card (below the source list) | `{ runId: string }` | Five panels occupy this slot, in `order`: (5) "Ask another question" (`src/slots/lens.evidence/ask-another-question.tsx`), C4's investigation directory -- every other panel below it is one of its routing targets (`src/data/askAnotherQuestion.ts` documents each question's real backing capability, or its absence); (10) "What did the model receive?" (`src/slots/lens.evidence/context-receipt.tsx`), delivery evidence via `src/data/received-context.ts` -- the exact rendered prompt stays in the older authorized context-receipt disclosure, mounted only after explicit user action; (20) "What mattered?" (`src/slots/lens.evidence/what-mattered.tsx`), the cross-linked prompt/answer-span heatmap; (25) "Are the claims supported?" (`src/slots/lens.evidence/claim-verification.tsx`), E1/E2/E3's deterministic claim segmentation and per-claim verification (`src/data/claimSupport.ts`); (30) "Why, and what to try" (`src/slots/lens.evidence/diagnosis-repair.tsx`), D1/D2's rule findings + narrative (`src/data/diagnosisRepair.ts`) plus D3's real preview/confirm/keep corrective-retry mechanics (`src/data/correctiveFlow.ts`, shared verbatim with `features/behavior/Behavior.tsx`'s own "Fix this answer" module -- see that data module's own doc comment for why it lives under `src/data/` rather than being duplicated). `data.runId` is the currently-selected run in Lens; every panel here owns and aborts its own fetches when that id changes, and none fires a POST except from an explicit click. Each panel renders only once `runId` is non-empty. Ask-another-question fires no fetch at all -- it only scrolls a sibling panel into view and records into `src/data/investigationHistory.ts`'s client-side history store. |
