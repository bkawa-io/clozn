...
THE SCHEMA (a fixed contract with the receipts system -- do not change the shape):
    {"id": "sec_rag_context", "name": "rag_context", "source": "api" | "auto" | "memory_card",
...
THREE WAYS A SECTION IS BORN, in priority order when more than one could apply to the same run:
  ...
  3. MEMORY CARD (`source: "memory_card"`) -- `memory_card_sections` locates each applied memory-card's
     ...
     concatenated onto that manifest (see `dedupe_ids` below) -- a card is a distinct, separately-toggle-
     able influence even when it happens to land inside a message an auto-chunk already sectioned.

ID UNIQUENESS. Each of the functions below guarantees unique `id`s WITHIN its own returned list (the
"sec_" + slug(name), suffixed _2/_3/... on collision rule the schema specifies). But callers that combine
more than one of these lists into a single run's `sections` field (explicit-or-auto, THEN append memory-
card sections -- see clozn.server.app._log_run) must re-check uniqueness across the COMBINED list, since
two independently-deduped lists can still collide with each other (a `clozn_section` literally named
"card_1", say). `dedupe_ids` does that one re-pass; run it last, once, over the final combined list.
...
# ======================================================================================================
# 4. applied memory cards -> sections: memory_card_sections
# ======================================================================================================

def memory_card_sections(assembled_messages, card_texts) -> list:
    """Locate each applied memory-card's TEXT inside `assembled_messages` (the run's recorded post-
    injection message list -- see memory_assembly.py / app.py's _log_run) by plain substring search, first
    match wins (document order), and emit one `source: "memory_card"` section per card found, named
    "card_1", "card_2", ... in the order `card_texts` was given (NOT necessarily document order -- these
    names track "which applied card", not "where it landed").

    A card whose text can't be found ANYWHERE (internalized memory mode has no assembled block to search at
    all; a block that got paraphrased/truncated between assembly and recording; an empty/whitespace-only
    card) is skipped SILENTLY -- per the spec, "never raise", and an unlocatable card is exactly the kind
    of soft miss that should degrade the manifest by one entry, not blow up the run record."""
    if not isinstance(assembled_messages, list) or not card_texts:
        return []
    used_ids: set = set()
    out = []
    for i, text in enumerate(card_texts, start=1):
        if not isinstance(text, str) or not text.strip():
            continue
        located = None
        for idx, msg in enumerate(assembled_messages):
        ...
        out.append({
            ...
            "name": name,
            "source": "memory_card",
            "parts": [{"message_index": idx, "start": pos, "end": pos + len(text)}],
            "char_count": len(text),
            "preview": text[:PREVIEW_CHARS],
        })
    return out
...
# ======================================================================================================
# 5. combining manifests from more than one source
# ======================================================================================================
...
# ======================================================================================================
# 6. resolving a section back to text (for tests, and for future receipts/inspector consumers)
# ======================================================================================================
